...
public class SdkUnitTests {
	...
	private static final String organizationId = System.getenv("ORGANIZATION_ID");
	...
	private static final String userImportFullPath = System.getProperty("user.dir") + "/src/test/docs/organization-user-import.csv";
	...
	public void GetOrganizationsTest() {
		...
		try {
			...
			System.out.println("Organizations: " + organizationsResponse);
		} catch (ApiException ex) {
			Assert.fail("Exception: " + ex);
		} catch (Exception e) {
			Assert.fail("Exception: " + e.getLocalizedMessage());
	...
	@Test
	synchronized public void BulkImportTest() {
		System.out.println("\nBulkImportTest:\n" + "===========================================");
		ApiClient apiClient = new ApiClient(BaseUrl);
		//String currentDir = System.getProperty("user.dir");
		...
		try {
			...
			OrganizationImportResponse organizationImportResponse = bulkImportsApi.createBulkImportAddUsersRequest(UUID.fromString(organizationId), userImportStr.getBytes());
			...
			bulkImportsApi.deleteBulkUserImport(UUID.fromString(organizationId), organizationImportResponse.getId());
		} catch (ApiException ex) {
			Assert.fail("Exception: " + ex);
		} catch (Exception e) {
			Assert.fail("Exception: " + e.getLocalizedMessage());
	...
	public void CreateUserTest() {
		...
		try {
			...
			PermissionProfileRequest permissionProfile = new PermissionProfileRequest()
					.id(9305351L);
			NewUserRequestAccountProperties account = new NewUserRequestAccountProperties()
					.id(UUID.fromString(accountId))
					.permissionProfile(permissionProfile);
			NewUserRequest request = new NewUserRequest()
					.addAccountsItem(account)
					.userName("testuser")
					.email("xxx@yyy.zzz");

			UsersApi usersApi = new UsersApi();
			NewUserResponse newUserResponse = usersApi.createUser(UUID.fromString(organizationId), request);
	...
	public void GetProductPermissionProfilesTest() {
		...
		try {
			...
			ProductPermissionProfilesApi productPermissionProfilesApi = new ProductPermissionProfilesApi();
			ProductPermissionProfilesResponse productPermissionProfilesResponse = productPermissionProfilesApi.getProductPermissionProfiles(UUID.fromString(organizationId), UUID.fromString(accountId));
	...
	public void GetUserProfilesTest() {
		...
		try {
			...
			OrganizationUsersResponse organizationUsersResponse = usersApi.getUsers(UUID.fromString(organizationId), getUsersOptions);
			...
			for (OrganizationUserResponse user: organizationUsersResponse.getUsers()) {
				UsersApi.GetUserProfilesOptions getUserProfilesOptions = usersApi.new GetUserProfilesOptions();
				getUserProfilesOptions.setEmail(user.getEmail());
				UsersDrilldownResponse usersDrilldownResponse = usersApi.getUserProfiles(UUID.fromString(organizationId), getUserProfilesOptions);
